...
pub fn solve(data: &str) -> Result<u32, &'static str> {
        ...
        match line {
            Some(x) => {
                if x.starts_with("seeds") {
                    ...
                    let seed_numbers = get_numbers(spl[1]);
    ...
    for seed in almanac.seeds {
        ...
        let mut dst = find_destination(almanac.seed_to_soil.clone(), seed);
        dst = find_destination(almanac.soil_to_fertilizer.clone(), dst);
        dst = find_destination(almanac.fertilizer_to_water.clone(), dst);
        dst = find_destination(almanac.water_to_light.clone(), dst);
        dst = find_destination(almanac.light_to_temperature.clone(), dst);
        dst = find_destination(almanac.temperature_to_humidity.clone(), dst);
        dst = find_destination(almanac.humidity_to_location.clone(), dst);

        println!("Seed to location: {} -> {}", seed, dst);
